...
import sys
from io import open
sys.path.append(r"D:\PyCharm Community Edition 2023.2.3\pythonProject\CENet - jo\CENet-main\networks\subnet")
import torch
from torch import nn
from torch.nn import CrossEntropyLoss, MSELoss, BCELoss
from CEmodule import CE
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, pos_tag_embedding = False, senti_embedding = False, polarity_embedding = False):
        ...
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0) #索引为0的向量其值置为0
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, pos_tag_ids=None, senti_word_ids=None, polarity_ids=None):
        ...
        if position_ids is None:
            position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device) #默认以0为起点
            position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids) #生成与input id维度一致的0
    ...
    def forward(self, hidden_states, visual=None, acoustic=None, visual_ids=None, acoustic_ids=None, attention_mask=None, head_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == ROBERTA_INJECTION_INDEX:
                hidden_states, kl_loss = self.CE(hidden_states, visual=visual, acoustic=acoustic, visual_ids=visual_ids, acoustic_ids=acoustic_ids)
        ...
        return outputs, kl_loss  # last-layer hidden state, (all hidden states), (all attentions)
...
class BertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):     #判断一个对象是否是已知的类型
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range) #标准正态分布
        elif isinstance(module, BertLayerNorm):
            module.bias.data.zero_()
            module.weight.data.fill_(1.0) #用1.0填充
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, visual=None, acoustic=None, visual_ids=None, acoustic_ids=None, pos_ids=None, senti_word_ids=None, polarity_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None):
        ...
        encoder_outputs, kl_loss = self.encoder(embedding_output, visual, acoustic, visual_ids, acoustic_ids,
