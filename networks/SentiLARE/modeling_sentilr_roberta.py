...
class RobertaEmbeddings(BertEmbeddings):
    ...
    def __init__(self, config, pos_tag_embedding = False, senti_embedding = False, polarity_embedding = False):
        ...
        self.padding_idx = 1
                                 #token type ids 表明属于哪个句子           pos tag 词性标签
    def forward(self, input_ids, token_type_ids=None, position_ids=None, pos_tag_ids=None, senti_word_ids=None, polarity_ids=None):
        ...
        if position_ids is None:
            ...
            position_ids = torch.arange(self.padding_idx+1, seq_length+self.padding_idx+1, dtype=torch.long, device=input_ids.device) #torch.arange(start=1, end=6) 1，2，3，4，5
            position_ids = position_ids.unsqueeze(0).expand_as(input_ids) #unsqueeze 升维   expand as 将position id 升维至与input id一致
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids, visual=None, acoustic=None, visual_ids=None, acoustic_ids=None, pos_tag_ids=None, 
        ...
        outputs, kl_loss = self.roberta(input_ids,
                               visual=visual,
                               acoustic=acoustic,
        ...
        return outputs, kl_loss  # (loss), logits, (hidden_states), (attentions)
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.kan1 = KAN([config.hidden_size, 512, config.hidden_size])
        self.kan2 = KAN([config.hidden_size, 256, config.num_labels])
