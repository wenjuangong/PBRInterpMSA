...
class act_function(torch.nn.Module):
    def __init__(self):
        super(act_function, self).__init__()
        self.W = torch.nn.Parameter(torch.tensor([0.5, 0.5, 0.5]),
                              requires_grad=True)
    def forward(self, x):
        weight = torch.tensor(0.01).to(DEVICE)
        x1 = torch.nn.functional.relu(x)
        x2 = torch.nn.functional.prelu(x, weight)
        x3 = torch.nn.functional.silu(x)
        return self.W[0]*x1+self.W[1]*x2+self.W[2]*x3
class KANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor): # 将输入数据通过模型的各个层，经过线性变换和激活函数处理，最终得到模型的输出结果
        ...
        base_output = F.linear(self.act_function(x), self.base_weight) # 计算基础线性层的输出  #self.base_activation(x)
